...
BEGIN
  ...
  INSERT INTO room_type (type_name, base_price, capacity) VALUES ('ZZTEST-Std-04', 1000, 2)
  ...
  INSERT INTO room_task (room_id, assigned_employee_id, description)
  VALUES (v_room, v_emp, 'ZZTEST clean after checkout') RETURNING roomtask_id INTO v_task;

  IF (SELECT status FROM room_task WHERE roomtask_id=v_task)='Pending'
  THEN RAISE NOTICE 'TEST 04-A1: PASS — new task is Pending';
  ELSE RAISE NOTICE 'TEST 04-A1: FAIL'; END IF;

  UPDATE room_task SET status='Completed', completed_time=now() WHERE roomtask_id=v_task;
  IF (SELECT status FROM room_task WHERE roomtask_id=v_task)='Completed'
     AND (SELECT completed_time FROM room_task WHERE roomtask_id=v_task) IS NOT NULL
  ...
  v_ok := FALSE;
  BEGIN
    UPDATE room_task SET completed_time = assigned_time - interval '1 hour' WHERE roomtask_id=v_task;
    v_ok := TRUE;
  ...
  INSERT INTO room_maintenance (room_id, reported_by, priority, description)
  VALUES (v_room, v_emp, 'High', 'ZZTEST broken AC') RETURNING room_maintenance_id INTO v_maint;
  UPDATE room_maintenance SET assigned_employee_id=v_emp, status='In Progress' WHERE room_maintenance_id=v_maint;
  UPDATE room_maintenance SET status='Completed', completion_date=CURRENT_DATE WHERE room_maintenance_id=v_maint;
  IF (SELECT status FROM room_maintenance WHERE room_maintenance_id=v_maint)='Completed'
  THEN RAISE NOTICE 'TEST 04-C: PASS — maintenance reported -> assigned -> completed';
  ELSE RAISE NOTICE 'TEST 04-C: FAIL'; END IF;

  -- ---------- 04-D: 'Under Maintenance' blocks the calendar ----------
  -- Spec (RBAC/status PDF, 10 Jul) allows 'Under Maintenance'; DDL v1 does not yet.
  BEGIN
    INSERT INTO room_availability (room_id, calendar_date, status)
    VALUES (v_room, '2026-10-10', 'Under Maintenance');
    IF (SELECT status FROM room_availability WHERE room_id=v_room AND calendar_date='2026-10-10')='Under Maintenance'
    THEN RAISE NOTICE 'TEST 04-D: PASS — calendar date marked Under Maintenance';
    END IF;
  EXCEPTION WHEN check_violation THEN
    RAISE NOTICE 'TEST 04-D: FAIL — DDL rejects ''Under Maintenance'' (KNOWN-FAIL: spec updated but DDL v1 CHECK list not yet — waiting on DDL v2)';
  END;
  -- TODO once booking function exists: assert reserving this room for 2026-10-10 is REJECTED.
  ...
  DELETE FROM room_maintenance WHERE room_maintenance_id=v_maint;
  DELETE FROM room_task WHERE roomtask_id=v_task;
